...
const sideEffectHandlers = {
  ...
  uiScale: (_, value) => {
    if (process.env.IS_ELECTRON) {
      platform.setZoomFactor(value / 100)
    }
  },
...
const customActions = {
  ...
  setupListenersToSyncWindows: ({ commit, dispatch }) => {
    if (process.env.IS_ELECTRON) {
      platform.handleSyncSettings((event, data) => {
        switch (event) {
      ...
      platform.handleSyncHistory((event, data) => {
        switch (event) {
          case SyncEvents.GENERAL.UPSERT:
      ...
      platform.handleSyncSearchHistory((event, data) => {
        switch (event) {
          case SyncEvents.GENERAL.UPSERT:
      ...
      platform.handleSyncProfiles((event, data) => {
        switch (event) {
      ...
      platform.handleSyncPlaylists((event, data) => {
        switch (event) {
          case SyncEvents.GENERAL.CREATE:
      ...
      platform.handleSyncSubscriptionCache((event, data) => {
        switch (event) {

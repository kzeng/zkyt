...
async function setNavigationHistoryDropdownOptions() {
  if (process.env.IS_ELECTRON) {
    ...
    const dropdownOptions = await platform.getNavigationHistory()
...
onMounted(() => {
  ...
  if (process.env.IS_ELECTRON) {
    ...
    platform.handleUpdateSearchInputText((searchQueryText) => {
      if (searchQueryText) {
...
onBeforeUnmount(() => {
  ...
  if (process.env.IS_ELECTRON) {
    ...
    platform.handleUpdateSearchInputText(null)
  }
})
</script>

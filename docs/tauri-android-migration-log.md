...
- Added a Tauri startup migration that enables backend fallback and keeps Android on Invidious as the preferred backend while Local API remains available as fallback. Local API is useful on VPN-enabled devices, but some youtubei.js browse/search paths can return empty or take too long on Android, so it should not block first-level page loading.

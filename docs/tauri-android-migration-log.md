...
- Follow-up Android playback diagnosis showed the play button was not the primary failure: Shaka had a player instance, but `getManifest()` was false, no variant tracks existed, the video element stayed at `readyState=0`, and no media segment requests were started.
- Replaced the Android Blob DASH manifest handoff with a Shaka `zkyt-dash-manifest:` networking scheme backed by an in-memory manifest map. This avoids WebView-specific `data:`/`blob:` manifest loading behavior while leaving all media segment URLs unchanged.
- Added a narrow DASH XML repair for generated manifests: only `<BaseURL>` text nodes are escaped for XML (`&`, `<`, `>`). Pixel testing still reported `Unexpected close tag` near the beginning of generated MPDs, which is consistent with unescaped URL text inside the XML manifest.
- Automated Android debugging with WebView DevTools showed a separate no-op play button state: the Shaka controls were mounted while `manifestSrc` was `null` and `legacyFormats` was empty. The video element therefore had no media source even though the page metadata had loaded.
- Updated Android Local API fallback behavior so a WEB player response that succeeds but returns no normal playable formats also retries with youtubei.js' ANDROID client. This matches the earlier precondition-error fallback path but covers the silent empty-format response.
- Guarded the watch-page renderer so the Shaka player only mounts when the selected format has a real source. Empty-source states now flow through fallback/error handling instead of showing inert playback controls.
- Added an Android-only YouTube embed fallback for the remaining no-normal-format case. This keeps the regular FreeTube/Shaka path for videos with playable URLs, but prevents the mobile watch page from getting stuck with inert controls when YouTube only returns SABR/no standard stream formats.
